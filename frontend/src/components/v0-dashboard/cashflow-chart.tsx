...
import { incomeExpense } from "@/v0-lib/data"
...
export function CashflowChart({ data = [] }: { data?: any }) {
  ...
  const net = data?.reduce((acc: number, curr: any) => acc + (curr.income || 0) - (curr.expense || 0), 0) || 0;
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-baseline gap-2">
              <span className={`tabular text-3xl font-semibold tracking-tight ${net >= 0 ? 'text-[var(--positive)]' : 'text-[var(--negative)]'}`}>
