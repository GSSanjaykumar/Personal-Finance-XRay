...
import { NotificationCenter } from "@/components/v0-dashboard/notification-center"
import { notifications } from "@/v0-lib/data"

export function Topbar({ onOpenCommand, user }: { onOpenCommand: () => void, user?: any }) {
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <button className="flex items-center gap-2.5 rounded-xl border border-[var(--border)] bg-[var(--surface)] py-1.5 pl-1.5 pr-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-[var(--border-strong)]">
            ...
            <span className="hidden text-left sm:block">
              ...
              <span className="block text-[11px] leading-tight text-muted-foreground">{user?.email || "Member"}</span>
            </span>
          </button>
        </div>
      </div>
