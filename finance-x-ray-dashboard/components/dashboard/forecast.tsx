...
const stats = [
  ...
  { label: "Proj. savings", value: "$18.4k" },
  { label: "Growth", value: "+28.7%" },
]
    ...
    <Reveal delay={0.1} className="h-full">
      <div className="flex h-full flex-col rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow-card)]">
        ...
        <div className="mt-5 flex items-baseline gap-2">
          <span className="tabular text-4xl font-semibold tracking-tight">$55.1k</span>
          <span className="text-sm text-muted-foreground">projected by Jan</span>
        </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3 border-t border-[var(--border)] pt-5">
          {stats.map((s) => (
            <div key={s.label}>
              ...
              <p className="tabular mt-1 text-sm font-semibold">{s.value}</p>
            </div>
          ))}
        </div>
        ...
        <button
          onClick={() =>
            toast({
              ...
              description: "Based on 12 months of income stability and a 34% savings rate, growth compounds to $55.1k.",
            })
          }
